...
export default function AppHeader() {
  ...
  function toggleNotifs() {
    ...
    setPopupMsg(newState ? t("notificationsOn") : t("notificationsOff"));
  }
  ...
  return (
    <>
      ...
      {/* Notification Popup */}
      <div 
        className={`fixed top-16 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 ease-out pointer-events-none ${
          popupMsg ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"
        }`}
      >
        <div className="bg-bg-card/90 backdrop-blur-sm border border-border shadow-lg shadow-black/50 text-text-primary text-sm font-bold px-4 py-2 rounded-full flex items-center gap-2">
          {notifsOn ? <Bell size={14} className="text-success" /> : <BellOff size={14} className="text-error" />}
          {popupMsg}
        </div>
      </div>
    </>
